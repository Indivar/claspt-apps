...
|-------|------|--------|-------|
| **Phase 0** | Shared preview bundle + mobile WebView migration | 3-4 days | Extract preview pipeline to `shared/preview/`, replace `react-native-marked` with WebView on mobile |
| **Phase 1** | Math (KaTeX), Callouts, Footnotes | 3-4 days | Marked extensions, no heavy libs |
| **Phase 2** | Mermaid, Wiki-links | 3-4 days | Mermaid is largest lib; wiki-links need autocomplete |
| **Phase 3** | Graphviz, Charts, TOC | 3-4 days | WASM loading, Chart.js canvas |
| **Phase 4** | Kanban, Spreadsheets, Timelines | 4-5 days | Custom interactive components with write-back |
| **Phase 5** | Music, Embeds, Presentations, Chemical | 2-3 days | Smaller scope, builds on Phase 1-4 patterns |
| **Phase 6** | Toolbar Row 2, Settings UI, Licenses page | 2-3 days | Wiring UI to extension registry + attribution |
| **Phase 7** | Editor-side widgets (P1: math, wiki-links, callouts) | 3-4 days | CodeMirror 6 ViewPlugin + Decoration |

**Total: ~24-30 days**

Phase 0 is the foundation — once the preview pipeline is shared, every extension built in Phases 1-5 works on desktop AND mobile automatically.
...
| File | Purpose |
|------|---------|
| `src/lib/extensions/registry.ts` | Extension registration, interface, enabled state |
| `src/lib/extensions/math.ts` | KaTeX inline + block Marked extension |
| `src/lib/extensions/mermaid.ts` | Mermaid fenced block renderer |
| `src/lib/extensions/callouts.ts` | Callout blockquote transformer |
| `src/lib/extensions/footnotes.ts` | Footnote Marked extension wrapper |
| `src/lib/extensions/wikilinks.ts` | Wiki-link preprocessor + click handler |
| `src/lib/extensions/graphviz.ts` | Viz.js WASM fenced block renderer |
| `src/lib/extensions/charts.ts` | Chart.js fenced block renderer |
| `src/lib/extensions/music.ts` | abcjs fenced block renderer |
| `src/lib/extensions/timelines.ts` | vis-timeline renderer |
| `src/lib/extensions/kanban.ts` | Kanban board renderer + write-back |
| `src/lib/extensions/spreadsheet.ts` | Spreadsheet grid + formula engine |
| `src/lib/extensions/toc.ts` | Table of contents generator |
| `src/lib/extensions/presentations.ts` | Slide separator + fullscreen presenter |
| `src/lib/extensions/embeds.ts` | Embed preprocessor + iframe sandbox |
| `src/lib/extensions/fenced-blocks.ts` | Shared fenced block routing for Marked |
| `src/components/editor/ExtensionToolbar.tsx` | Row 2 toolbar with dynamic buttons |
| `src/stores/extension-store.ts` | Zustand store for extension enabled state |
| `src/components/LicensesPanel.tsx` | Open Source Licenses page listing all third-party dependencies |
| `src/lib/licenses.ts` | License data: package names, versions, licenses, source URLs |
| `shared/preview/index.ts` | Shared preview pipeline: Marked instance factory, DOMPurify config, extension hooks |
| `shared/preview/extensions.ts` | Extension registry + lazy loaders (consumed by desktop and mobile WebView) |
| `shared/preview/preview.html` | Standalone HTML shell for mobile WebView (loads shared pipeline via bundled JS) |
| `mobile/src/components/MarkdownWebView.tsx` | WebView wrapper: loads shared preview HTML, bridges postMessage for navigation/media |

**Modified files:**

| File | Change |
|------|--------|
| `src/components/MarkdownPreview.tsx` | Use shared preview pipeline from `shared/preview/`, add post-render hook |
| `src/components/editor/EditorToolbar.tsx` | Import and render `ExtensionToolbar` as Row 2 |
| `src/components/SettingsPanel.tsx` | Add Markdown Extensions section + link to Licenses page |
| `src/components/EditorPane.tsx` | Pass extension settings to CodeMirror |
| `src/index.css` | Callout styles, KaTeX overrides, extension container styles |
| `package.json` | Add katex, mermaid, @viz-js/viz, chart.js, abcjs, vis-timeline, marked-footnote, js-yaml |
| `mobile/src/screens/PageViewScreen.tsx` | Replace `react-native-marked` with `MarkdownWebView` |
| `mobile/src/screens/PageEditScreen.tsx` | Replace `react-native-marked` preview with `MarkdownWebView` |
| `mobile/package.json` | Add `react-native-webview`, remove `react-native-marked` |

### 11.3 Dependencies to Add

| Package | Version | Size | License | Used By |
|---------|---------|------|---------|---------|
| `katex` | ^0.16 | ~90KB | MIT | Math |
| `marked-katex-extension` | ^5 | ~2KB | MIT | Math Marked integration |
| `mermaid` | ^11 | ~500KB | MIT | Diagrams |
| `@viz-js/viz` | ^3 | ~400KB | MIT + **CPL 1.0** | Graphviz (see 11.4) |
| `chart.js` | ^4 | ~200KB | MIT | Charts |
| `abcjs` | ^6 | ~200KB | MIT | Music |
| `vis-timeline` | ^7 | ~150KB | Apache-2.0 OR MIT | Timelines |
| `vis-data` | ^7 | ~30KB | Apache-2.0 OR MIT | Timelines (peer dep) |
| `marked-footnote` | ^1 | ~3KB | MIT | Footnotes |
| `js-yaml` | ^4 | ~20KB | MIT | Chart/Timeline config parsing |

### 11.4 Licensing

All dependencies use permissive licenses (MIT or Apache-2.0 OR MIT) except one:

**`@viz-js/viz` (Graphviz WASM):** The npm wrapper is MIT, but it bundles Graphviz compiled to WebAssembly, which is licensed under the **Common Public License 1.0** (CPL). CPL is a weak copyleft — commercial use in closed-source apps is permitted with these obligations:

1. Include the CPL 1.0 license text in an accessible location
2. Preserve Graphviz copyright notices
3. Provide a way for users to obtain the Graphviz source (link to the [Graphviz GitLab repo](https://gitlab.com/graphviz/graphviz))

**Resolution:** A new **Open Source Licenses** page in Settings will list all third-party libraries with their licenses and source links. This satisfies the CPL attribution requirement and is good practice for all bundled dependencies.

### 11.5 Open Source Licenses Page

New page accessible from Settings → About → Open Source Licenses.

**Content:** Auto-generated or manually maintained list of all third-party packages, their versions, licenses, and links to source repositories. Grouped by category (Markdown Extensions, Core, Build Tools).

**Implementation:**
- New component: `src/components/LicensesPanel.tsx`
- Data source: static JSON or TypeScript constant listing packages
- Accessible from Settings panel (link in About/footer section)
- Includes full license text for CPL 1.0 (Graphviz) and MIT (bulk)
