# Claspt with Windsurf

1. Install Claspt, open your vault, and enable the local API (Settings > Integrations > Local API).
2. Run:

   ```bash
   claspt mcp install windsurf --secrets
   ```

   which writes the server into Windsurf's MCP config with a token minted for it.
3. In Cascade, the Claspt tools are available; start with `memory_guide` and `bootstrap_project` for a new project.
