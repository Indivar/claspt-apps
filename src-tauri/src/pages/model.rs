...
//! Page data model and Markdown (de)serialization.
//!
//! Defines the core [`Page`] / [`PageMeta`] types and the summary structs used by
//! list views, plus the pure functions that convert between an on-disk `.md` file
//! and those types: [`parse_page`] / [`serialize_page`] (YAML frontmatter framed by
//! `---` delimiters), [`generate_filename`] (the `YYYY-MM-DD-HHMMSS-title-slug.md`
//! convention), and [`make_snippet`] (a preview string with secret values masked).
